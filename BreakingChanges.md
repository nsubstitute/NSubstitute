...
If it is not possible for you to use a C# 7-compatible compiler, we have added a compatibility wrapper around `Arg` called `CompatArg`, which has all the same members as `Arg` just without the `ref` return type. If you replace `Arg.` references with `CompatArg.` in your project then you can continue to use the old compiler with NSubstitute 4.x. You will also need to reference `NSubstitute.Compatibility` for these files (replace `using NSubstitute;` with `using NSubstitute; using NSubstitute.Compatbility;`). To make this easier and less verbose you may want to create a wrapper within the root of your test project with a shorter name (e.g. `ArgC`) that delegates to `CompatArg` (this will also avoid needing the extra `using` import).
