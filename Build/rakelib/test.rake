...
task :test => [:compile] do
    ...
    sh "#{NUNIT_EXE} #{tests} /nologo /framework=net-4.0 /exclude=Pending /xml=#{OUTPUT_PATH}/UnitTestResults.xml"
end
...
task :specs => [:compile] do
    ...
    sh "#{NUNIT_EXE} #{tests} /nologo /framework=net-4.0 /exclude=Pending /xml=#{OUTPUT_PATH}/SpecResults.xml"
end
...
task :pending => [:compile] do
    ...
    sh "#{NUNIT_EXE} #{acceptance_tests} /framework=net-4.0 /nologo /include=Pending /xml=#{OUTPUT_PATH}/PendingSpecResults.xml"
end
